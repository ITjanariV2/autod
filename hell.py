...
# mycursor.execute("CREATE TABLE auto24 (id INT AUTO_INCREMENT PRIMARY KEY, title VARCHAR(255), year VARCHAR(255), km VARCHAR(255), fuel VARCHAR(255), type VARCHAR(255), bodytype VARCHAR(255), drive VARCHAR(255), image VARCHAR(255))")
arrayToJson = []
amount = 0
page = 0
array = []
...
def scrape(page):
    ...
    for x in content:
        ...
        #     if 'model-short' in span.get('class', []) or 'transmission_short_icon' in span.get('class', []):
        #         continue
        #     arr.append(span.text.strip())
        # array.append(arr)
        ...
        # for g in filtered_array:
        #     if g == 'B':
        #         filtered_array.pop(filtered_array.index(g))
    ...
    for image in img:
        ...
        # print(imgSource)
        # print(str(len(allImages)) + ' images')
    ...
    # allCars.append(filtered_array)
        # print(str(len(filtered_array)) + ' aa')
            # print(str(len(allCars)) + ' cars dashsbdbahsbddhuasbd')
        # elif len(filtered_array) >= 8:
            # print(allImages.)
            # print(str(carCountForImages) + ' count ahahahhaha')
            
            # del allImages[carCountForImages]
            # print(carCountForImages)
    ...
    #     # print(len(car))
    #     if len(car) != 7:
    #         print(allCars.index(car))
    #         allImages.pop(allCars.index(car))
    #         allCars.pop(allCars.index(car))
    for i in range(len(allCars) - 1, -1, -1):
        if len(allCars[i]) != 7 or 'B' in allCars[i] or 'D' in allCars[i]:
            # print(str(allCars[i]) + ' diajjidijasjidjasjdijdauidjdddddddddddddddddddddd')
            allImages.pop(i)
            allCars.pop(i)
    ...
    for i1, i2 in zip(allCars, allImages):
        ...
        # print(len(obj['description']))
        # print(len(arrayToJson))
        # print(i1) midagi on m2da
        # print(i1)
            

        # for i1, i2, i3 in zip(title, description, img):
        #     # ma olen v2ga v2sinud (ma vabandan)
        #     i1 = list(i1)
        #     i2 = list(i2)
        #     i1 = i1[1:]
        #     i1.pop(2)
